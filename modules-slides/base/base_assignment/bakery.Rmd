...
### Let's name our baking bot! {-}

Copy the code below to your R script to create a bot called `whisky`.
...
Now our baking bot needs a friend. Let's create a `bot2`. Go ahead and name it anything you like. 
```{r names2, eval=F}
...
<p style="color: black; opacity: 0.85;"> Black is the result or output of the code. In the top case, the `[1]` in front means there was item returned for `bot`, and its value was `Whisker`. </p>
...
_Hint: You are allowed to re-run your code. You can even click the **Run** button at the top to re-run ALL of your script._
...
Everything has a name in R and you can name things almost anything you like, even `TOP_SECRET_SHHHH...` or `data_face_99`.

But there are a _few_ restrictions. R doesn't like names to include spaces or special characters found in math equations, like `+`, `-`, `*`, `\`, `/`, `=`, `!`, or `)`. 
